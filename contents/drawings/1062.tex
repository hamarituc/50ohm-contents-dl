...
\begin{circuitikz}
    ...
    \draw(mic.south)
        to [adc, >] ++(2,0)
        node [sourcecodershape, anchor=west, fill={DARCblue!50!white}](foo){};
    \draw(foo.east)
        to [channelcoder, >, fill={DARCblue!50!white}] ++(2,0)
        node [inputarrow](){}
        node [mappershape, anchor=west, fill={DARCblue!50!white}](question){};
