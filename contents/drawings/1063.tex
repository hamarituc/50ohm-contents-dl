...
\begin{circuitikz}
    ...
    \draw(adc1.east) ++(1,0) coordinate(h1)
        (h1|-mix.west)
        node[demappershape, anchor=west, fill={DARCblue!50!white}](question){};
    ...
    \draw(question.east)
        to [channeldecoder, >, fill={DARCblue!50!white}] ++(2,0)
        node[sourcedecodershape, anchor=west, fill={DARCblue!50!white}](source){}
        node[inputarrow](){};
    \draw(source.east)
        to [dac, >] ++(2,0)
        node[inputarrow](){}
